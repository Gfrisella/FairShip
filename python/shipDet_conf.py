...
def addScoringPlane(anindex=0, xpos=0.0, ypos=0.0, zpos=0.0, xhalfw=500.0, yhalfh=500.0, lz=0.1, medium_name="iron",
     ...
     scoringplane = ROOT.ScoringPlane(izstring, ROOT.kTRUE, ROOT.kFALSE, xhalfw, yhalfh, lz)
     ...
     print(f"    defined {izstring} at x,y,z = {xpos} , {ypos} , {zpos} cm ({shape_info}, medium: {medium_name})")
...
def configure_snd_mtc(yaml_file, ship_geo):
    ...
    if mtc_geo.zPosition == "auto":
        ...
        mtc_geo.zPosition = ship_geo.muShield.Z[-1]
...
def configure(run, ship_geo, stepwise=False, stepLenght=0.5, staircase=False, SND_scoring_planes = False, HeBalloon = False):
    ...
    if PutScoringPlanes:
        ...
        ScoPlane_arb8_dz = [0] * len(ScoPlane_Add)  # NEW: Half-length in Z for Arb8
    ...
    if HeBalloon:
        ...
        ScoPlane_shape.extend(["Box"] * 2 + ["Arb8"] * 5)

        balloon_thickness +=0.01  # cm, to avoid overlap with the muon shield
        Helium_balloon = [
    ...
    if SND_scoring_planes:
        ...
        z_SND_planes = [ val for i in range(1, ship_geo.muShield.nMagnets) for val in np.linspace(ship_geo.muShield.Z[i] - ship_geo.muShield.Z_rel[i], ship_geo.muShield.Z[i] + ship_geo.muShield.Z_rel[i], int(np.ceil(2.0 * ship_geo.muShield.Z_rel[i] / stepLenght)) + 1)]
    ...
    for iz in range(len(ScoPlane_zpos)):
        if ScoPlane_Add[iz]:
            # Prepare arb8_corners depending on whether dz is zero
            ...
            scoringplane = addScoringPlane(
                ...
                arb8_corners=arb8_corners  # Now passed as a proper argument
            )
    ...
    #exclusionList = []
    #exclusionList = ["Muon","Ecal","Hcal","Strawtubes","TargetTrackers","NuTauTarget","HighPrecisionTrackers", "Veto","Magnet","MuonShield","TargetStation","NuTauMudet","EmuMagnet", "TimeDet", "UpstreamTagger"]
    exclusionList = ["Muon","Ecal","Hcal","TargetTrackers","NuTauTarget","HighPrecisionTrackers",
                     "Veto","NuTauMudet","EmuMagnet", "UpstreamTagger"]
