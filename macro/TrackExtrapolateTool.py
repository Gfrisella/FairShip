...
def extrapolateToPlane(fT,z,minNdf = 20):
  parallelToZ = ROOT.TVector3(0., 0., 1.)
  z_ecal = 100*u.m
...
# test for fit status for each point
   ...
   #print(NewPosition,parallelToZ)
    pos,mom = fstate.getPos(),fstate.getMom()
   #print("zs = ",zs,"   pos,mom = ",pos.X(),pos.Y(),pos.Z(),mom.X(),mom.Y(),mom.Z())
    rep.setPosMom(state,pos,mom)
    detPlane = ROOT.genfit.DetPlane(NewPosition, parallelToZ)
    detPlanePtr = ROOT.genfit.SharedPlanePtr(detPlane)
    try:
      result = rep.extrapolateToPlane(state, detPlanePtr)
     #result = rep.extrapolateToPlane(state, NewPosition, parallelToZ ) THIS CRASHES !!!
      #state.Print()
      pos,mom = state.getPos(),state.getMom()
      # print("rep.extrapolateToPlane(state, NewPosition, parallelToZ) returned",result)
      # print("new pos,mom = ",pos.X(),pos.Y(),pos.Z(),mom.X(),mom.Y(),mom.Z())
      rc = True
    except:
   #  print('error with extrapolation: z=',z,"  pos,mom = ",pos.X(),pos.Y(),pos.Z(),mom.X(),mom.Y(),mom.Z())
      pass
    if not rc or z>z_ecal:
     if not rc:   print("FAILED: rc =", rc)
     if z>z_ecal: print("FAILED: z =", z," > z_ecal = ", z_ecal)
     sys.exit(-1)
